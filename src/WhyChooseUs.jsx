...
const WhyChooseUs = () => (
  <section className="w-full flex flex-col items-start py-20 px-2 md:px-8 bg-white animate-fadeInUp">
    ...
    <div className="w-full max-w-7xl grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8 ml-0 md:ml-9">
      {cards.map((card, idx) => (
        <div key={idx} className={`rounded-2xl shadow-lg ${card.bg} flex flex-col items-start px-6 py-8 md:px-8 md:py-10 min-h-[340px] md:min-h-[440px] transition-transform hover:-translate-y-2 w-full hover:card-pop`}>
          <img src={card.icon} alt={card.title} className="h-16 w-16 mb-6 hover:img-zoom" />
          <h3 className="text-[#1661A6] text-2xl md:text-[25.08px] font-cormorant font-semibold mb-4 text-left leading-tight ">{card.title}</h3>
          ...
          <a href="https://wa.me/7017094225" target="_blank" rel="noopener noreferrer">
            <button className="mt-auto bg-[#0655A0] text-white font-inter font-medium rounded-full px-8 py-2 text-[15.68px] shadow hover:bg-[#0d3c6e] transition text-left hover:btn-glow">Join Now</button>
          </a>
        </div>
      ))}
    </div>
