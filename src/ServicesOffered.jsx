...
const ServicesOffered = () => (
  <section className="w-full flex flex-col items-start py-10 md:py-16 px-2 md:px-8 bg-white relative overflow-hidden animate-fadeInUp">
  ...
  <div className="w-full max-w-7xl mx-auto flex flex-col md:grid md:grid-cols-2 md:grid-rows-2 gap-4 md:gap-10 mt-2 md:mt-5 relative z-10">
    {/* Top left Card */}
    <div className={`relative rounded-2xl p-4 md:p-8 ${services[0].bg} transition-transform hover:scale-105 hover:shadow-xl cursor-pointer flex flex-col items-start justify-center w-full h-full md:aspect-square order-1 hover:card-pop`}> 
      ...
      <a href="https://calendly.com/meenalkhandelwal55">
      <button className="mt-4 md:mt-6 bg-[#0655A0] hover:bg-blue-800 text-white md:text-[18px] px-5 md:px-7 py-2 md:py-3 rounded-full shadow-lg transition font-bold leading-[24px] px-6 md:px-8 text-[15.68px] tracking-[2px] text-center uppercase font-urbanist hover:btn-glow">
        BOOK A CALL
      </button> </a>
    </div>

    {/* Top right Image with overlay */}
    <div className="relative rounded-2xl overflow-hidden transition-transform hover:scale-105 hover:shadow-xl cursor-pointer w-full h-68 md:h-full md:aspect-square order-2 hover:card-pop">
      <img src={services[1].img} alt={services[1].alt} className="object-cover w-full h-full mix-blend-multiply hover:img-zoom" />
      <div className="absolute inset-0 bg-black/20"></div>
    </div>

    {/* Bottom left Image with overlay */}
    <div className="relative rounded-2xl overflow-hidden transition-transform hover:scale-105 hover:shadow-xl cursor-pointer w-full h-68 md:h-full md:aspect-square order-4 md:order-3 hover:card-pop">
      <img src={services[2].img} alt={services[2].alt} className="object-cover w-full h-full mix-blend-multiply hover:img-zoom" />
      <div className="absolute inset-0 bg-black/20"></div>
    </div>

    {/* Bottom right Card */}
    <div className={`relative rounded-2xl p-4 md:p-8 ${services[3].bg} transition-transform hover:scale-105 hover:shadow-xl cursor-pointer flex flex-col items-start justify-center w-full h-full md:aspect-square order-3 md:order-4 hover:card-pop`}>
    <div className='bg-[#E7F6A0] justify-end px-10 py-10 md:px-20 md:py-20 rounded-full'></div>
